 ...
 * The bindings map returned by the contraint validation will contains those object returned by the JavaScript function, with its values mapped to
...
public record JsConstraintViolation(EvaluationContext context, String constraintMessage, JsLibrary library, String functionName, Map<String, Object> bindings)
   ...
   public String message() {
      ...
      for ( final Map.Entry<String, Object> entry : bindings.entrySet() ) {
         String value = "";
         if ( entry.getValue() instanceof Node_Literal literal ) {
            value = literal.getLiteralLexicalForm();
         } else if ( entry.getValue() instanceof Node_URI namedNode ) {
