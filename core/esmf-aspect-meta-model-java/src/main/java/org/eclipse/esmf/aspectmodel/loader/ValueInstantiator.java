...
public class ValueInstantiator {
    ...
    * @param languageTag if the datatype is rdf:langString, this must be set to the corresponding
    *        language tag, otherwise it can be null
    * @param datatypeUri the URI of the datatype that describes the value
    * @return the ScalarValue instance or empty if the lexical representation can not be parsed
   ...
   /**
    * Creates new instances of {@link ScalarValue} from the value representation in RDF, with base
    * attributes
    */
   public Optional<ScalarValue> buildScalarValue( final MetaModelBaseAttributes baseAttributes, final String lexicalRepresentation,
      ...
      // 1. A custom parser for rdf:langString values can not be registered with Jena, because it would
      // only receive from Jena during
      // parsing the lexical representation of the value without the language tag (this is handled
      // specially in Jena).
      // 2. This means that a Literal we receive here which has a type URI of rdf:langString will be of
      // type org.apache.jena.datatypes
