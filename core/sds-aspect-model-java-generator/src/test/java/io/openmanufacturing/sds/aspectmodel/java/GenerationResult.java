...
class GenerationResult {
   ...
   void assertNumberOfFiles( final int expectedNumberOfFiles ) {
      ...
      assertThat( compilationUnits )
            .withFailMessage(
                  numAllFiles + " file(s) found, but " + numFailedCompilationUnits + " could not be parsed due to "
                        + parseProblems )
                  ...
                  field -> {
                     ...
                     if ( fieldTypeOrTypeName instanceof String ) {
                        ...
                        assertThat( actual ).is( anyOf( new Condition<TypeToken>( expected::equals, "fully equal" ),
                              new Condition<>( token -> token.toString().equals( expected.toString() ),
                                    expected.toString() ) ) );
                     }
                  } );

      if ( !annotations.isEmpty() ) {
         assertThat( fields ).allSatisfy(
               field -> assertThat( annotations.get( field.resolve().getName() ) ).isEqualTo(
   ...
   void assertConstructor( final String className, final ImmutableMap<String, Object> fieldTypesOrTypeNames,
      ...
      final Optional<ConstructorDeclaration> constructor = compilationUnits.get( className )
   ...
   void assertConstructorParameters( final NodeList<Parameter> parameters,
         final ImmutableMap<String, Object> fieldTypesOrTypeNames,
         final Map<String, String> expectedParameterAnnotations ) {
      assertThat( parameters ).hasSize( fieldTypesOrTypeNames.size() );

      if ( expectedParameterAnnotations.size() > 0 ) {
         assertThat( parameters ).allSatisfy(
               parameter -> assertThat( expectedParameterAnnotations.get( parameter.resolve().getName() ) ).isEqualTo(
   ...
   void assertConstructorBody( final String className, final Set<String> expectedbody ) {
      ...
      assertThat( expectedbody.stream().collect( Collectors.joining() ) )
            .contains( bodyStatements.stream().map( Node::toString ).collect( Collectors.joining() ) );
   }
   ...
   private TypeToken<?> typeTokenForField( final FieldDeclaration field ) {
      return TypeToken.of( resolveRecursive( TypeResolution.toResolve( field.resolve().getType().asReferenceType() ) )
            .getResolved() );
   }
   ...
   private TypeResolution resolveRecursive( final TypeResolution toResolve ) {
      ...
      try {
         ...
         rawType = generatedClasses.entrySet().stream().filter(
               entry -> entry.getKey().toString().equals( toResolve.getClassNameToResolve() ) )
   ...
   void assertEnumConstants( final String className, final Collection<String> expectedConstants,
      ...
      final List<EnumConstantDeclaration> constants = compilationUnits.get( className )
   ...
   void assertNamespace( final String className, final String javaPackageName ) {
      assertThat( compilationUnits ).containsKey( className );
      assertThat( compilationUnits.get( className ).getPackageDeclaration().get().getName().toString() )
            .isEqualTo( javaPackageName );
   }
