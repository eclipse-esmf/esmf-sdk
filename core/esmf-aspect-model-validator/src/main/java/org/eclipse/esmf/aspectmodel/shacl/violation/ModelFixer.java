...
/**
 * Function that takes a model (which includes violations) and a list of applicable fixes as input
 * and produces a new model with the fixes applied
 */
public class ModelFixer implements BiFunction<Model, List<Fix>, Model> {
   ...
   private record ModelChange(
         Model toAdd, Model toRemove
   ) {
      ModelChange() {
   ...
   private record FixVisitor(
         Model oldModel
   ) implements Fix.Visitor<ModelChange> {
      @Override
