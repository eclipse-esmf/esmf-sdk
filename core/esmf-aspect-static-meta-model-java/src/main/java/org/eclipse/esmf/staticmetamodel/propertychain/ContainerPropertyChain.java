...
/**
 * A {@link PropertyChain} that holds chain elements of type {@link ContainerProperty}, e.g.
 * {@link Optional} or {@link Collection}. The first container type within the chain is the one
 * propagated to the end of the chain, i.e. if somewhere an {@code Optional} appears, the final
 * property type will also be an optional. The same holds true for {@code Collection}s in between.
 * However, independent from the collection types that are found within the chain, the final
 * collection type will always be a {@link List}.
 *
