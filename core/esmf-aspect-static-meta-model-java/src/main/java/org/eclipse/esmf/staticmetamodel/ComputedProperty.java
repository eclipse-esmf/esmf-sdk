...
/**
 * Allows ad-hoc definitions of static properties that wrap another property and compute their value
 * using a given function, taking the original property value as an input. The computed property
 * value also can be of a different type. A typical example would be a unit conversion, e.g.
 * transforming a property value that is given in {@code kg} into a value that is given in
 * {@code gram}, by using the function {@code v -> v * 1000}. Using computation functions that use
 * further input it is also possible to create dynamic computed properties. It is however not
...
public class ComputedProperty<C, R> extends StaticProperty<C, R> {
   ...
   /**
    * Creates a {@code ComputedProperty} from the given {@link StaticProperty}, applying the given
    * {@link Function}.
    *
   ...
   /**
    * Creates a {@code ComputedProperty} from the given {@link DefaultProperty} and an accompanying
    * {@link PropertyAccessor}, applying the given {@link Function}.
    *
    * @param wrappedProperty the property to wrap
    * @param accessor the property accessor to use on the wrapped property, as a
    *        {@code DefaultProperty} does not allow property access
    * @param computation the computation function to apply to the property value
    * @param <C> the type containing the wrapped property
    * @param <T> the type of the wrapped property
    * @param <R> the type of the computed property, i.e. of the computation result
