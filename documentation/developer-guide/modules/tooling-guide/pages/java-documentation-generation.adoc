...
== Generating Sample Apache Parquet Payload

The sample Parquet payload generator is used to create a valid Apache Parquet payload for an Aspect model as it could be returned by
an Aspect that implements that Aspect model. This follows the
xref:samm-specification:ROOT:payloads.adoc#mapping-to-parquet[mapping rules] as defined in the Meta Model specification.
The generator uses `samm:exampleValue`s of Properties if present, otherwise random values corresponding to the
respective data types are generated.
...
[source,java,indent=0,subs="+macros,+quotes"]
----
include::example$GenerateParquetPayload.java[tags=imports]
----
++++
</details>
++++
...
[source,java,indent=0,subs="+macros,+quotes"]
----
include::example$GenerateParquetPayload.java[tags=generate]
----
...
and https://github.com/asyncapi/spec/blob/master/spec/asyncapi.md[3.1.0] of the AsyncAPI specification.
...
.Generate AsyncAPI with optional features
[source,java,indent=0,subs="+macros,+quotes"]
----
include::example$GenerateAsyncApi.java[tags=generateWithFeatures]
----

See xref:samm-cli.adoc#aspect-to-asyncapi[the SAMM CLI reference] for the full list of available feature
flags and their effect on the generated specification.
