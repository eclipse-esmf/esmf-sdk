...
.24+| [[aspect-to-openapi]] aspect <model> to openapi | Generate https://spec.openapis.org/oas/v3.0.3[OpenAPI] specification
                                   ...
                                   | _--operations-api-path_, _-oap_ : Set the path for the JSON-RPC endpoint, to which
                                       Aspect's operations are mapped (default: `/rpc-api/vX`, where X is the Aspect Model
                                       major version)                                                                        |
                                   ...
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
.8+| [[aspect-to-asyncapi]] aspect <model> to asyncapi | Generate https://www.asyncapi.com/docs/reference/specification/v3.1.0[AsyncAPI] specification for an Aspect Model| `samm aspect AspectModel.ttl to asyncapi`
                                   | _--output, -o_ : output file path (default: stdout)                                     |
                                   ...
                                   | _--separate-files, -sf_ : Create separate files for each schema                         |
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
.6+| [[aspect-to-json]] aspect <model> to json | Generate example JSON payload data for an Aspect Model                      | `samm aspect AspectModel.ttl to json`
                                   | _--output, -o_ : output file path (default: stdout)                                     |
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
...
.4+| [[aspect-to-schema]] aspect <model> to schema | Generate JSON schema for an Aspect Model                                | `samm aspect AspectModel.ttl to schema`
                                   ...
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
.11+| [[aspect-to-sql]] aspect <model> to sql | Generate SQL script that sets up a table for data for this Aspect            | `samm aspect AspectModel.ttl to sql`
                                   | _--output, -o_ : output file path (default: stdout)                                     |
                                   ...
                                   | _--decimal-scale, -ds_ : The scale to use for Databricks decimal columns
                                     ...
                                     the xref:java-aspect-tooling.adoc#databricks-type-mapping[Databricks type mapping].     |
                                   | _--custom-column, -col_ : Additional custom column definition, e.g. for databricks
                                     following the pattern `column_name DATATYPE [NOT NULL] [COMMENT 'custom']`. This
                                     parameter can be repeated for multiple columns.                                         | `samm aspect AspectModel.ttl to sql --custom-column "column_name STRING NOT NULL COMMENT 'custom'"`
...
.2+| [[package-export]] package <model or namespace URN> export | Exports an Aspect Model with its dependencies
                                     or a complete namespace to a Namespace Package (.zip)                                   | `samm package urn:samm:org.eclipse.example.myns:1.0.0 export --output package.zip` +
                                     {empty} +
                                   ...
                                   | _--output, -o_ : output file path (default: stdout); as ZIP is a binary format, it is
                                       strongly recommended to output the result to a file by using the -o option or the
                                       console redirection operator '>')|
.5+| [[import]] import <file> <URN> | Imports (i.e., translates) a file to an Aspect Model. The only currently supported
                                      format is JSON Schema.                                                                  | `samm import MyModel.schema.json urn:org.example:1.0.0#MyModel --output-directory models`
                                    | _-js, --json-schema_ : If the input file name does not end with .schema.json, use this
                                      switch to force interpreting the input as JSON schema.                                  |
                                    | _-d, --output-directory_ : output directory to write files to                           |
                                    | _-e, --entity_ : Create an Entity instead of an Aspect from the imported schema         |
                                    | _-t, --todo_ : Add TODO descriptions to otherwise undescribed model elements            |
...
will not know where to find the corresponding file(s).

NOTE: The `--models-root`, `--custom-resolver` and `--github` switches (and their related options)
are options of the respective subcommand, not of the top-level `samm` command. They must therefore
be placed after the command and subcommand name, e.g., `samm aspect AspectModel.ttl validate
--custom-resolver myresolver.sh`, not before it (`samm --custom-resolver myresolver.sh aspect
AspectModel.ttl validate` will not work).
...
The template is a document in the OpenAPI format, with one crucial difference - the presence of a templated Paths Object
`paths.$$__DEFAULT_QUERIES_TEMPLATE__$$`.
Values not defined by the generator will be taken from the template.
For selected generator-defined Aspect Model operations, the values can be taken from the corresponding _Operation Object_ template
`paths.$$__DEFAULT_QUERIES_TEMPLATE__$$`.

This operation template is currently applied to generated read endpoints, query endpoints and operations endpoints.

Most fields in these Operation Object templates are merged with the generated operations they apply to.

The `responses` field has special merge semantics for these generated operations:

- If the operation template does not define `responses`, the generated default responses are used:
`200`, `400`, `401`, `403` and `404`.

- If the operation template defines `responses`, the template responses replace the generated error responses.
Generated error responses such as `400`, `401`, `403` and `404` are not added unless they are explicitly defined in the template.

- The generated success response is preserved by default because its schema reference depends on the generated operation.
For read and query endpoints this response references the generated Aspect response.
For operations endpoints this response references `#/components/responses/OperationResponse`.

- A template-defined `responses."200"` replaces the generated success response.

- Set `x-esmf-generate-success-response: false` on the operation template to prevent adding the generated success response.
This flag only affects the generated success response and only when custom `responses` are defined.
Responses explicitly defined in the template, including `200`, `201`, `204`, `5xx` or `default`, are kept.
...
The following template replaces the generated error responses for generated GET operations while keeping the generated success response:
...
To replace the generated success response, define `responses."200"` explicitly:
...
paths:
  __DEFAULT_QUERIES_TEMPLATE__:
    get:
      responses:
        "200":
          description: The resource was returned successfully with a custom response.
----

To provide only template-defined responses, for example `201` without the generated success response:
...
Each Aspect is associated with one https://www.asyncapi.com/docs/reference/specification/v3.1.0#channelObject[channel address].
...
is `/\{tenant-id}/{namespace}/{version}/{aspect-name}`, with each placeholder later resolved to a corresponding channel parameter (see below).
...
----
{
 "channels" : {
    "Movement" : { // <3>
      "address" : "/{tenant-id}/{namespace}/{version}/{aspect-name}", // <3>
...
Each of the `tenant-id`, `namespace`, `version` and `aspect-name` channel parameters is rendered as a full
https://www.asyncapi.com/docs/reference/specification/v3.1.0#parameterObject[Parameter Object] only when the
corresponding placeholder (`\{tenant-id}`, `{namespace}`, `{version}`, `{aspect-name}`) is literally present in the
channel address, so the `parameters` section always matches what the address actually uses. If no explicit channel
address is configured via `--channel-address`/`-ca`, the default address
`/{tenant-id}/{namespace}/{version}/{aspect-name}` is used, so all four parameters are included. If an explicit
channel address is configured, only the placeholders it contains result in a channel parameter; any placeholder
missing from the address is omitted.
...
https://www.asyncapi.com/docs/reference/specification/v3.1.0#operationObject[operations] section,
channel
https://www.asyncapi.com/docs/reference/specification/v3.1.0#messagesObject[messages] section and
https://www.asyncapi.com/docs/reference/specification/v3.1.0#schemaObject[schemas] section:
