...
.24+| [[aspect-to-openapi]] aspect <model> to openapi | Generate https://spec.openapis.org/oas/v3.0.3[OpenAPI] specification
                                   ...
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
.9+| [[aspect-to-asyncapi]] aspect <model> to asyncapi | Generate https://www.asyncapi.com/docs/reference/specification/v3.1.0[AsyncAPI] specification for an Aspect Model| `samm aspect AspectModel.ttl to asyncapi`
                                   | _--output, -o_ : output file path (default: stdout)                                     |
                                   ...
                                   | _--feature-flags, -ff_ : Enable optional AsyncAPI generation behaviors (comma-separated).
                                       Valid values: `AddTenantIdInChannelParameters`, `ExpandChannelParameters`             | `samm aspect AspectModel.ttl to asyncapi -ff AddTenantIdInChannelParameters`
...
The `responses` field is an exception for those generated operations:

- If `responses` is specified for an operation template,
it replaces the generated responses object completely for the generated operation.

- If `responses` is omitted, the generated default responses are used.

No automatic success status code is added when custom `responses` are specified.
...
To replace the generated responses for generated GET operations that use the operation template,
specify the complete `responses` object in the template:
...
The AsyncAPI generator supports two opt-in behaviors, enabled via the `--feature-flags`/`-ff` option
(samm-cli) or the `featureFlags` parameter (Maven plugin). Both are disabled by default, in which case
generator output is unchanged from the examples above.

===== `AddTenantIdInChannelParameters`

Adds a `tenant-id` channel parameter. If no explicit channel address is configured (via
`--channel-address`/`-ca`), the derived address is additionally prefixed with `/{tenant-id}`:

[source,shell,subs="attributes+,+quotes"]
----
samm aspect _Movement.ttl_ to asyncapi -ff AddTenantIdInChannelParameters
----
...
----
{
  "channels" : {
    "Movement" : {
      "address" : "/{tenant-id}/movement/0.0.1/Movement",
      "description" : "Channel for updating Movement Aspect.",
      "parameters" : {
        "namespace" : "movement",
        "version" : "0.0.1",
...
If an explicit channel address is configured and contains the literal `{tenant-id}` placeholder, the
address is left untouched and only the parameter is added:

[source,shell,subs="attributes+,+quotes"]
----
samm aspect _Movement.ttl_ to asyncapi -ca "/{tenant-id}/custom/path" -ff AddTenantIdInChannelParameters
----

If the explicit channel address does *not* contain the `{tenant-id}` placeholder, the flag has no effect
and no `tenant-id` parameter is added.

===== `ExpandChannelParameters`

Changes the `namespace`, `version` and `aspect-name` channel parameters (and `tenant-id`, if present) from
plain string values into AsyncAPI https://www.asyncapi.com/docs/reference/specification/v3.1.0#parameterObject[Parameter Objects]:

[source,shell,subs="attributes+,+quotes"]
----
samm aspect _Movement.ttl_ to asyncapi -ff ExpandChannelParameters
----
...
Both flags can be combined, e.g. `-ff AddTenantIdInChannelParameters,ExpandChannelParameters`.
