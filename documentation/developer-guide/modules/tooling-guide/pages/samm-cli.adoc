...
.16+| aspect <model> to openapi    | Generate https://spec.openapis.org/oas/v3.0.3[OpenAPI] specification for an Aspect Model| `samm aspect AspectModel.ttl to openapi -j`
                                   ...
                                   | _--custom-resolver_ : use an external resolver for the resolution of the model elements |
.6+| aspect <model> to asyncapi    | Generate https://www.asyncapi.com/docs/reference/specification/v3.0.0[AsyncAPI] specification for an Aspect Model| `samm aspect AspectModel.ttl to asyncapi`
                                   | _--output, -o_ : output file path (default: stdout)                                     |
                                   | _--channel-address, -ca_ : Use this param make possible provide channel address.
                                   ...
                                   | _--application-id, -ai_ : Use this param for provide application id.                    |
                                   | _--semantic-version, -sv_ : use the full semantic version from the Aspect Model as the
                                       version for the Aspect API                                                            |
...
As xref:using-the-cli-to-create-a-json-openapi-specification[OpenAPI], every AsyncAPI specification is based on one Aspect.
which needs a separately defined https://www.asyncapi.com/docs/reference/specification/v3.0.0#channelObject[channel address], this is not required.
The channel address will be defined as string with the `-ca` option, i.e.: /123-456/789-012/test/1.0.0/Aspect. The default channel address, using the above
defined `--channel-address`, would result in /{namespace}/\{version}/<aspectName>.
...
:Movement a samm:Aspect ; # <3>
   samm:name "Movement" ;
   samm:preferredName "MovementPrefferedName"@en ; # <2>
...
----
{
  "asyncapi" : "3.0.0",
  "info" : {
    "title" : "MovementPrefferedName MQTT API", // <2>
    "version" : "v2", // <1>
...
The name of the Aspect is used to generate several important AsyncAPI artifacts, like the channel definitions for the API:
...
AsyncAPI specification is generated using operations and events.
This section describe only operations.
If the Aspect also has a non-empty list of operations defined, like the one in the following example:
...
:GetSpeed a samm:Operation ;
   samm:preferredName "GetSpeedPrefName"@en ;
   samm:description "GetSpeed summary"@en ;
   samm:input ( :GetSpeedInput ) ; <1>
   samm:output :GetSpeedOutput . <2>

:GetSpeedOutput a samm:Property ;
   samm:preferredName "GetSpeedOutputPrefferedName"@en ;
   samm:description "GetSpeedOutput desc"@en ;
   samm:characteristic :OutputCharacteristic .

:OutputCharacteristic a samm:Characteristic ;
   samm:preferredName "OutputCharacteristicPrefferedName"@en ;
   samm:description "OutputCharacteristic desc"@en ;
   samm:dataType :OutputEntity .

:OutputEntity a samm:Entity ;
   samm:name "GetSpeedOutput" ;
   samm:preferredName "GetSpeedOutputPrefferedName"@en ;
   samm:description "GetSpeedOutput desc"@en ;
...
:GetSpeedInput a samm:Property ;
   samm:name "getSpeedInput" ;
   samm:preferredName "getSpeedInputPrefferedName"@en ;
   samm:description "getSpeedInput desc"@en ;
   samm:characteristic samm-c:Text . <4>
----
  ...
  "components" : {
    "messages" : {
      "GetSpeedInput" : { // <1> <4>
        ...
        "title" : "getSpeedInputPrefferedName",
        "summary" : "getSpeedInput desc",
      ...
      "GetSpeedOutput" : { // <2> <4>
        ...
        "title" : "GetSpeedOutputPrefferedName",
        "summary" : "GetSpeedOutput desc",
        "content-type" : "application/json",
        "payload" : {
    ...
    "schemas" : {
      "GetSpeedInput" : { // <1> <3>
        "type" : "string",
        "description" : "getSpeedInput desc"
      },
      "GetSpeedOutput" : { // <2> <3>
        "type" : "string",
        "description" : "GetSpeedOutput desc"
      }
    }
  }
}
...
This section describe mapping between Aspect Events and AsyncAPI spec.
...
:SpeedUpdateEvent a samm:Event ;
   samm:preferredName "speedUpdate"@en ;
   samm:description "This is event for update speed property"@en ;
   samm:parameters ( :updatedSpeed :propertyName ) .

:updatedSpeed a samm:Property ;
   samm:preferredName "updatedSpeedPreferredName"@en ;
   samm:description "Updated Property Desc."@en ;
   samm:see <http://example.com/> ;
   samm:see <http://example.com/me> ;
   samm:exampleValue "1234567890" ;
   samm:characteristic samm-c:Text .

:propertyName a samm:Property ;
   samm:preferredName "propertyNamePreferredName"@en ;
   samm:description "PropertyName Desc"@en ;
   samm:see <http://example.com/> ;
   samm:see <http://example.com/me> ;
   samm:exampleValue "1234567890" ;
   samm:characteristic samm-c:Text .
      ...
      "SpeedUpdateEvent" : { // <1>
        ...
        "properties" : {
          "updatedSpeed" : { // <3> <4>
            "title" : "updatedSpeed",
            "type" : "string",
            "description" : "Updated Property Desc."
          },
          "propertyName" : { // <3> <4>
            "title" : "propertyName",
            "type" : "string",
            "description" : "PropertyName Desc"
          }
        }
      }
    }
