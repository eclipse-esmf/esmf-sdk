...
----
{
 "channels" : {
    "Movement" : { // <3>
      "address" : "/{tenant-id}/movement/0.0.1/Movement", // <3>
...
The `namespace`, `version` and `aspect-name` channel parameters are always rendered as full
https://www.asyncapi.com/docs/reference/specification/v3.1.0#parameterObject[Parameter Objects]. If no
explicit channel address is configured via `--channel-address`/`-ca` (Maven: `channelAddress`), the
derived address is always prefixed with the `/{tenant-id}` placeholder, matching the behavior of the
OpenAPI generator, and a corresponding `tenant-id` channel parameter is added. If an explicit channel
address is configured, the `tenant-id` parameter is only added when that address itself contains the
literal `{tenant-id}` placeholder; otherwise it is omitted.
