...
.13+| aspect <model> to openapi         | Generate https://spec.openapis.org/oas/v3.0.3[OpenAPI] specification for an Aspect Model             | `bamm aspect AspectModel.ttl to openapi -j`
                                        ...
                                        |  _--comment, -c_ : only in combination with --json; generates `$comment` OpenAPI 3.1 keyword for all `bamm:see` attributes|
...
@prefix : <urn:bamm:com.mycompany.myproduct:2.0.0#> . <1>
