...
ERR_EMPTY_EXAMPLE_VALUE - Cannot automatically generate an example value for a property in the JSON payload because the associated regular expression constraint is too complex or unsupported.

   Note: This error applies only to JSON payload generation.

Resolution:

1. Add an example value manually for this property.
2. Simplify the regular expression so that an example can be generated automatically.
3. Use the `--fail-on-empty-example-value` command-line option to enable strict validation. When this option is enabled, the tool will throw `ERR_EMPTY_EXAMPLE_VALUE` instead of generating an empty example value for properties with complex regular expressions.
