...
| `operationsApiPath` | The path for the JSON-RPC endpoint, to which Aspect's operations are mapped. | `String` | `/rpc-api/vX`, where X is the Aspect major version | {nok}
...
| `featureFlags` | A list of optional AsyncAPI generation behaviors to enable. Valid values: `AddTenantIdInChannelParameters`, `ExpandChannelParameters`. | `List<String>` | none (empty) | {nok}
| `useSemanticApiVersion` | Use the complete semantic version of the Aspect Model as the version of the Aspect API. | `Boolean` | `false` | {nok}
...
| `addTypeAttribute` | Adds a `@type` attribute for inherited Entities | `Boolean` | `false` | {nok}
|===

=== Generating Sample Parquet Payload

The `generateParquetPayload` goal generates a valid sample Apache Parquet payload for a given Aspect Model as it
could be returned by an Aspect that implements that Aspect Model. The default life cycle phase for
the goal is `generate-resources`.
...
<build>
  <plugins>
    <plugin>
      ...
      <executions>
        <execution>
          <id>generate-parquet-payload</id>
          <goals>
            <goal>generateParquetPayload</goal>
          </goals>
        </execution>
      </executions>
      <configuration>
...
| `githubServerId` | The id of the `<server>` entry describing the GitHub repository to resolve models from. | `String` | none | {nok}
| `includes` | A list of Aspect Model URNs identifying the Aspect Models to be included in the plugin execution. | `String` | none | {ok}
| `outputDirectory` | The path to the directory where the generated Parquet payload will be written to. | `String` | none | {ok}
| `addTypeAttribute` | Adds a `@type` attribute for inherited Entities | `Boolean` | `false` | {nok}
|===
...
https://docs.oracle.com/en/java/javase/25/docs/api/java.base/java/util/ServiceLoader.html[ServiceLoader].
